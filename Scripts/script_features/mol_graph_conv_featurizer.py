...
from deepchem.feat.molecule_featurizers.dmpnn_featurizer import get_atom_mass
...
def _construct_bond_feature(bond: RDKitBond, distance_matrix) -> np.ndarray:
  ...
  ## Se non funziona rimuovere questo e la definizione di distance_matrix nella dichiarazione del metodo
  dist = get_bond_graph_distance_one_hot(bond, distance_matrix)
...
class MolGraphConvFeaturizer(MolecularFeaturizer):
  ...
  def _featurize(self, datapoint: RDKitMol, **kwargs) -> GraphData:
    ...
    # se non funziona rimuovere questa riga
    distance_matrix = GetDistanceMatrix(datapoint)
    ...
    if self.use_edges:
      features = []
      for bond in datapoint.GetBonds():
        features += 2 * [_construct_bond_feature(bond, distance_matrix)]
